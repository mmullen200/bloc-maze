...
var maps = [        "   XXXXXXXX     XXXXXXXX      \n" +
					...
					"   X      XX   XXXXXXTTXX  XXX\n" +
					"   X      XTT  TXXXTTTTXX  X  \n" +
					"   XX     XTT  TXXTTTTTXX  XXX\n" +
					"XXXXX     XTT  TXTTTTTTX     X\n" +
					"X      XTTXTTTTTTTTTTTTX     X\n" +
					"X     XXTTTTTTTTXTTTTTXX     X\n" +
...
// We added this so we can dynamically generate mesh maps
function generateMeshMap() {
    
    //array of maps
    map = maps[currentLevel].split("\n");
    ZSIZE = map.length * HORIZONTAL_UNIT;
    XSIZE = map[0].length * HORIZONTAL_UNIT;
    
    meshMap = new Array(map.length);
    ...
    addVoxel = (function() {
        ...
        for (var i = 0; i < 8; i++) {
            materials.push(new THREE.MeshPhongMaterial({
                color: new THREE.Color().setHSL(Math.random() * 0.3 + 0.3, 0.5, Math.random() * 0.25 + 0.75)
            }));
        }
        function material() {
        ...
        var EXIT = new THREE.CubeGeometry(HORIZONTAL_UNIT, VERTICAL_UNIT, HORIZONTAL_UNIT);

        var exitMaterial = new THREE.MeshPhongMaterial({color: 0xff00ff, shininess: 50});
        ...
        return function(type, row, col) {
            ...
            switch(type) {
                ...
                case 'X':
                    ...
                    break;
                case 'W':
                    mesh = new THREE.Mesh(EXIT.clone(), exitMaterial);
                    mesh.scale.y = 1.2;
                    mesh.position.set(x, VERTICAL_UNIT*1.5, z);
                    break; 
            }
	...
	return function(player) {
		...
		switch (cell.char) {
			...
			case 'X':
				...
				break;
            case 'W':
            //    currentLevel--; // just for debug purposes
                hitAnExitBlock();
                break;
		}
	};
